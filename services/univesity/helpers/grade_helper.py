...
class GradeHelper(BaseHelper):
    ...
    def delete_grade(self, grade_id) -> requests.Response:
        response = self.api_utils.delete(self.ID_PREFIX.format(grade_id))
        return response

    def put_grade(self, grade_id, json: dict) -> requests.Response:
        response = self.api_utils.put(self.ID_PREFIX.format(grade_id), json=json)
        return response

    def get_grade(self, grade_id) -> requests.Response:
        response = self.api_utils.get(self.ID_PREFIX.format(grade_id))
        return response
