...
class TeacherHelper(BaseHelper):
    ...
    def delete_teacher(self, teacher_id) -> requests.Response:
        response = self.api_utils.delete(self.ID_TEACHER.format(teacher_id))
        return response

    def put_teacher(self, teacher_id, json: dict) -> requests.Response:
        response = self.api_utils.put(self.ID_TEACHER.format(teacher_id), json=json)
        return response

    def get_teacher(self, teacher_id) -> requests.Response:
        response = self.api_utils.get(self.ID_TEACHER.format(teacher_id))
        return response
