...
class GroupHelper(BaseHelper):
    ...
    def put_group(self, group_id, json) -> requests.Response:
        response = self.api_utils.put(self.ID_GROUP.format(group_id), json=json)
        return response

    def get_group(self, group_id) -> requests.Response:
        response = self.api_utils.get(self.ID_GROUP.format(group_id))
        return response

    def delete_group(self, group_id) -> requests.Response:
        response = self.api_utils.delete(self.ID_GROUP.format(group_id))
        return response
