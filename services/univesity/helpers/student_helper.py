...
class StudentHelper(BaseHelper):
    ...
    def delete_student(self, student_id) -> requests.Response:
        response = self.api_utils.delete(self.ID_STUDENT.format(student_id))
        return response

    def put_student(self, student_id, json: dict) -> requests.Response:
        response = self.api_utils.put(self.ID_STUDENT.format(student_id), json=json)
        return response

    def get_student(self, student_id) -> requests.Response:
        response = self.api_utils.get(self.ID_STUDENT.format(student_id))
        return response
