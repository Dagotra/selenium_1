...
class SearchGamePage(BasePage):
    ...
    PRICE_GAME = By.XPATH, "(//div[contains(@class, 'search_discount_and_price')]//div[@data-price-final])[{}]"
    ATTRIBUTE_PRICE = "data-price-final"
    PRICE_LOCATOR = By.XPATH, "//div[contains(@class,'discount_block') and @data-price-final]"
    DYNAMIC_LOC_NAME_GAME = By.XPATH, "(//a[contains(@href, 'app')]//span[@class='title'])[{}]"
    ...
    @staticmethod
    def format_locator(locator: tuple[str, str], index: int) -> tuple[str, str]:
        xpath, value = locator
        format_locator = xpath, value.format(index)
    ...
    def sort_descending_order(self):
        ...
        sort_selector.click()
        price_button = self.wait.until(EC.element_to_be_clickable(self.DESC_PRICE_BUTTON))
        price_button.click()
    ...
    def get_price_game(self, index):
        price = self.wait.until(EC.visibility_of_element_located(self.format_locator(self.PRICE_GAME, index)))
        return price.get_attribute(self.ATTRIBUTE_PRICE)

    def get_name_in_list_game(self, index):
        name_game = self.wait.until(
            EC.visibility_of_element_located(self.format_locator(self.DYNAMIC_LOC_NAME_GAME, index))).text
        return name_game
